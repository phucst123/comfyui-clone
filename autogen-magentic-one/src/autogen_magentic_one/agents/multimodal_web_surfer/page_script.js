var MultimodalWebSurfer = MultimodalWebSurfer || (function() {
  ...
  let getInteractiveElements = function() {
    ...
    for (let i=0; i<nodeList.length; i++) { // Copy to something mutable
        if (!nodeList[i].hasAttribute("__elementId")) {
          nodeList[i].setAttribute("__elementId", "" + (nextLabel++));
        }
    ...
    for (let i=0; i<nodeList.length; i++) {
       ...
       if (results.indexOf(node) == -1) {
           if (!node.hasAttribute("__elementId")) {
               node.setAttribute("__elementId", "" + (nextLabel++));
               results.push(node);
           }
    ...
    // Find elements containing "Download" text
      nodeList = document.querySelectorAll("*");
      for (let i = 0; i < nodeList.length; i++) {
          let node = nodeList[i];
          
          // Check if element contains Download text
          if (node.textContent.toLowerCase().includes('download')) {
              ...
              while (element && element !== document.body) {
                  if (element.onclick || 
                      ...
                      element.className.includes('button') ||  // Thêm check cho class có chứa button
                      element.className.includes('btn')) {     // Thêm check cho class có chứa btn
                      
                      let ariaLabel = element.getAttribute("aria-label");
                    if (ariaLabel === null) {
                            element.setAttribute("aria-label", node.textContent);
                    } else {
                            element.setAttribute("aria-label", node.textContent + " " + ariaLabel);
                    }
                      if (!element.hasAttribute("__elementId")) {
                              element.setAttribute("__elementId", "" + (nextLabel++));
